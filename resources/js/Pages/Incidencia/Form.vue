...
const numeroIncidencias = ref(0);
const incidenciasUsuarioHoy = props.incidenciaAll.filter((incidencia) => {
    return incidencia.dni === "12345678";
});
console.log("incidencias hoy", incidenciasUsuarioHoy.length);
// Método para contar incidencias
const contarIncidencias = (dniUsuario) => {
    // Filtrar las incidencias del usuario con el DNI dado y que ocurrieron hoy
    const incidenciasUsuarioHoy = props.incidenciaAll.filter((incidencia) => {
        return incidencia.dni === dniUsuario;
    });
    console.log("cantidadIncidencia", incidenciasUsuarioHoy);
    // Contar el número de incidencias realizadas por el usuario hoy
    numeroIncidencias.value = incidenciasUsuarioHoy.length;
    alert(numeroIncidencias.value);
};

console.log(props.incidenciaAll);

// const incidenciaRepetida = () => {
//     props.incidenciaAll.
// }
const form = useForm({
    id: null,
...
const ConfirmDialog = () => {
    Swal.fire({
        ...
        const incidenciasUsuarioHoy = props.incidenciaAll.filter(
            (incidencia) => {
                return incidencia.dni === form.dni;
            }
        );
        console.log("incidencias hoy", incidenciasUsuarioHoy.length);
        if (incidenciasUsuarioHoy.length > 3) {
            // alert("Maykol deja de gritar!!!!!!!!!!!!!!!!!!!!!");
            Swal.fire({
                title: "Supero el límite de incidencias por día",
                icon: "error",
            }).then(() => {
                // Redireccionar a la página después de cerrar el cuadro de diálogo
...
const datosCompletos = computed(() => {
    ...
    const oficinaSeleccionada = form.oficina_id
        ? props.oficinas.find((oficina) => oficina.id === form.oficina_id)
              .nombre
        : "No seleccionada";
...
const ifdatosOtros = computed(() => {
    const tipoProblemaSeleccionado = form.tipo_problema_id
        ...
        : "No seleccionado";
    return (
        tipoProblemaSeleccionado === "Otros" ||
        tipoProblemaSeleccionado === "otros"
    );
});
console.log(3 === 3);
const submitForms = () => {
    const dniNuevo = form.dni;
    console.log(dniNuevo);
    const incidenciasUsuarioHoy = props.incidenciaAll.filter((incidencia) => {
        return incidencia.dni === "12345678";
    });
    // console.log("incidencias hoy", incidenciasUsuarioHoy.length);
    if (3 === 3) {
        alert("Supero el limite de incidencias");
        return;
    }
    if (areCamposRequeridosLlenos()) {
        if (3 === 3) {
            alert("Cuslquier cosa - Maykol");
        } else {
            form.post(`/ayuda`);
        }
    } else {
                        ...
                        <div class="form">
                            <div class="md:space-y-2 mb-3">
                                ...
                                <p class="text-red-600 text-center">
                                    Tiempo restante para la recarga:
                                    {{ tiempoRestante }} segundos
                                </p>
                            </div>
                            <form
                                @submit.prevent="submitForms"
                            ...
                            >
                                ...
                                <div class="mb-6">
                                    ...
                                    <p
                                        :class="{
                                            'text-red-500': validationErrorDNI,
                                        }"
                                        v-if="validationErrorDNI"
                                    >
                                        {{ validationErrorDNI }}
                                    </p>
                                </div>
